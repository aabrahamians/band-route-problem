window.addEventListener('load', () => {
    ...
    getPath = (e) => {
        ...
        if (isBrute) {
            ...
            trip.shuffleAndTest()
            console.log('best Distance:', trip.bestDistance)
            console.log('best Route:', trip.bestRoute)
            updateBruteResults( {
                runFrequency,
                bestRoute: trip.bestRoute,
                bestDistance: trip.bestDistance
            })
            drawPath(trip.bestRoute)
        } else {
            updateBruteResults()
            // this method uses nearest neighbor
            var trip = new tripCalculator(selectedDataSet);
            // find nearest neighbors starting from home
            trip.connectNearestCities();
            drawPath(trip.visited)
        }


    };

    //clear canvas iterater over paths array, draw dot and line to next node
    // @param {pathNodes[]} array of places
    drawPath = (pathNodes) => {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.beginPath();
        ctx.moveTo(pathNodes[0].x, pathNodes[0].y);
        ctx.fillStyle = "#FF0000";
        ctx.fillRect(pathNodes[0].x, pathNodes[0].y, 15, 15); // fill in the pixel at (10,10)

        for (each in pathNodes) {
            ctx.lineTo(pathNodes[each].x, pathNodes[each].y);
            ctx.fillStyle = "#FF0000";
            ctx.fillRect(pathNodes[each].x, pathNodes[each].y, 5, 5); // fill in the pixel at (10,10)
            ctx.stroke();
        }
    }
    //show results of brute force || clear results if using other method
    updateBruteResults= (data)=>{
        let message= "";
        if(data){
         let  { runFrequency, bestDistance} = data;
            message = `Randomized ${runFrequency} times: best distance was ${bestDistance}`
        }

        document.getElementById('results').innerHTML= message
    }
    //event listener for dropdown
    document.getElementById('brute').addEventListener('click', getPath);
    document.getElementById('nearestN').addEventListener('click', getPath);
